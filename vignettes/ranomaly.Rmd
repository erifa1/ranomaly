---
title: "rANOMALY Tutorial"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{ranomaly}
...
install_miss_package <- function(package) {
  if (!requireNamespace(package, quietly = TRUE)) {
    install.packages(package)
  } else {
    message(paste("Le package", package, "est déjà installé."))
  }
}

install_miss_package("DT")
install_miss_package("pander")

library(DT)
library(pander)

devtools::load_all(".")
load("./data/use_case.rdata")
```
...
* Process raw sequences from Illumina paired end or IonTorrent single end sequencing.

* ASV (Amplicon Sequence Variant) computation thanks to `dada2` algorithm
...
rANOMALY is full described in the publication:
...
Each function has a detailed help accessible in R via `?{function}`. All functions are documented in the [Reference section](https://umrf.pages.mia.inra.fr/ranomaly/reference/index.html) of this website
...
sm <- read.table("data/sample_metadata.csv", sep="\t",header=TRUE)
DT::datatable(sm)
```

```{r echo = FALSE}
load("data/robjects.Rdata")
```
...
```{r}
DT::datatable(read.table("data/dada2_out/read_tracking.csv",sep="\t",header=TRUE), filter = "top")
```
...
![](data/decontam_out/venndiag_filtering.png)
...
[ExploreMetabar](https://explore-metabar.sk8.inrae.fr/)
...
beta_strain = diversity_beta_light(data_decontam, col = "strain", cov="time", dist0 = "bray", ord0 = "MDS", output="./plot_div_beta_strain/", tests = TRUE)
beta_time = diversity_beta_light(data_decontam, col = "time", cov="strain", dist0 = "bray", ord0 = "MDS", output="./plot_div_beta_time/", tests = TRUE)
# ggplotly(beta_strain$plot), ggplotly(beta_time$plot), 
```


```{r beta, warning=FALSE, results = 'hide', eval = FALSE}
...
```{r,warning = FALSE, message = FALSE,  fig.keep='all', fig.align='left', fig.width = 15, fig.height = 10, echo = TRUE, results = 'hide', eval = FALSE}
...
```{r,warning = FALSE, message = FALSE, echo = TRUE, fig.width = 15, results = 'hide', eval = FALSE}
...
```{r,warning = FALSE, message = TRUE,  fig.keep='all', fig.align='left', echo = TRUE, results = "hide", eval = FALSE}
...
```{r,warning = FALSE, message = TRUE,  fig.keep='all', fig.align='left', fig.height = 10, echo = TRUE, results = "hide", eval = FALSE}
...
```{r,warning = FALSE, message = TRUE,  fig.keep='all', fig.align='left'}
DT::datatable(resF$table, filter = "top", options = list(scrollX = TRUE), rownames= FALSE)
```
...
```{r pslda, results = 'hide', warning = FALSE, message = FALSE, eval = FALSE}
...
```{r heatmap, fig.align='center', results = "hide", eval = FALSE}
...
```{r asvenn, results = "hide", eval = FALSE}
...
![](data/cytoscape/cytoscape_anomaly.png)
