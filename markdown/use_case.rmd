...
etc...
Those must match the sample names stored in the sample metadata file.
...
Our function integrates the basics ASV frequency (nb_reads_ASV/nb_total_reads) and overall prevalence (nb_sample_ASV/nb_total_sample) filtering. We included an option to filter out ASV based on they taxa names (for known recurrent contaminants).
...
- Kronas plot before and after filtering.
...
* Cytoscape
`phy2cyto_fun` allows to generate input files for cytoscape which is usefull to vizualise shared taxa. You can see below an example of representation with Cytoscape on our data. Each nodes and arrows position and aesthetic can be easily modified. 

![cytoscape](/home_pers/rifa/Images/cytoscape_anomaly.png)
