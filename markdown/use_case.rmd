...
beta_souche = diversity_beta_light(data, col = "souche", cov="temps", dist0 = "bray", ord0 = "MDS", output="./plot_div_beta_souche/", tests = TRUE)
beta_temps = diversity_beta_light(data, col = "temps", cov="souche", dist0 = "bray", ord0 = "MDS", output="./plot_div_beta_temps/", tests = TRUE)

beta_souche_temps = diversity_beta_light(data, col = "souche_temps", dist0 = "bray", ord0 = "MDS", output="./plot_div_beta_souche_temps/", tests = TRUE)
```
### Ordination plot


```{r beta_ordination, warning=FALSE}
htmltools::tagList(list(ggplotly(beta_souche$plot), ggplotly(beta_temps$plot), ggplotly(beta_souche_temps$plot)))
```
...
pander(beta_souche$permanova)
```

* pairwise permanova on concatenated factors

```{r beta_test2, warning=FALSE}
pander(beta_souche_temps$pairwisepermanova)
```
...
- Comparaison 1
```{r,warning = FALSE, message = TRUE,  fig.keep='all', fig.align='left', fig.width = 15, fig.height = 10, echo = TRUE}
 out1$sauvage_t0_vs_mutant_t0$signif
```

- Comparaison 2
```{r,warning = FALSE, message = TRUE,  fig.keep='all', fig.align='left', fig.width = 15, fig.height = 10, echo = TRUE}
 out1$sauvage_t50_vs_mutant_t50$signif
```
...
plsda_res <- plsda_fun(data = data, output = "./plsda_family/", column1 = "souche_temps", rank = "Family")
```
...
heatmap_plot = heatmap_fun(data = data, column1 = "souche_temps", top = 20, output = "./plot_heatmap/", rank = "Species")
```
...
outvenn = ASVenn_fun(data = data,output = "./ASVenn/", rank = "ASV", column1 = "souche_temps", shared = TRUE)
```
