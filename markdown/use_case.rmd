...
etc...
The sample names are extracted from the file name. We consider as sample name anything that is before the first underscore. This must match the sample names that are in sample metadata files.
...
* `./dada2_out/read_tracking.csv` that summarize the read number after each filtering step.

* `./dada2_out/raw_otu-table.csv`
