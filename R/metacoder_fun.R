...
metacoder_fun <- function(data = data, output = "./metacoder", column1 = "", column2 = "", rank = "Species",
                          signif = TRUE, plottrees = FALSE, min ="1000", comp = "all"){
  ...
  if(column2 != ""){
    ...
    for (var in vector){
      ...
      flog.info('Done.')
      title = paste('Comparison ',var,' ',column2,rank,sep='')
      gg <- launch_metacoder(psobj, column2, rank, var, title, plot1=plottrees)
      plots = marrangeGrob(grobs=gg[1:2],nrow=1,ncol=2)
      ggsave(paste0(output,'/metacoder_',column2,'_',var,'_',rank,'.png'),plot=plots, width=30, height=16, dpi = 320)
    }
  }else {
    if(comp == "matrix"){
      flog.info("Comparison in matrix...")
      title = paste('Matrix column ',column1,' at rank ',rank,sep='')
      gg <- launch_metacoder(data, column1, rank, '', title, plot1=plottrees)
      ...
      for (comp in (1:ncol(combinaisons))){
        ...
        pp <- launch_metacoder(psobj, column1, rank, '', title, plot1=plottrees)
